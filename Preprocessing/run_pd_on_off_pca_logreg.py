...
def main():
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(gkf.split(X_flat, y, groups=groups), start=1):
        ...
        # store how many PCs survived in this fold
        pca_model = pipe.named_steps["pca"]
        n_pcs_used = pca_model.n_components_
    ...
    summary_row = {
        "dataset": DATASET_NAME,
        "n_folds": len(fold_df),
        "mean_n_pcs_used": fold_df["n_pcs_used"].mean(),
        "std_n_pcs_used": fold_df["n_pcs_used"].std(),
    ...
    fold_out = OUTPUT_DIR / f"{DATASET_NAME}_pca_fold_results.csv"
    summary_out = OUTPUT_DIR / f"{DATASET_NAME}_pca_summary.csv"
