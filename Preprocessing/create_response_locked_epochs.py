...
OUTPUT_DIR = Path("/Users/derived_epochs_response")
...
def get_output_epoch_path(subject_id, session_id):
    return OUTPUT_DIR / f"sub-{subject_id}_ses-{session_id}_task-{TASK_NAME}_resp-epo.fif"
...
def load_trial_table(subject_id, session_id):
    ...
    if KEEP_ONLY_RESPONDED and "response_exists" in df.columns:
        df = df[df["response_exists"] == 1].copy()

    df = df[df["response_onset"].notna()].copy()
    df = df.sort_values("response_onset").reset_index(drop=True)
...
def build_events_array(raw, trial_df, event_codes):
    ...
    samples = np.round(trial_df["response_onset"].to_numpy(dtype=float) * sfreq).astype(int) + first_samp
...
def create_response_locked_epochs(subject_id, session_id):
    print(f"\n=== Response-locked epoching: sub-{subject_id}, ses-{session_id} ===")
    ...
    if len(trial_df) == 0:
        print("  -> No valid response-locked trials after filtering. Skipping.")
        return
...
def main():
    ...
    for subject_id, session_id in pairs:
        try:
            create_response_locked_epochs(subject_id, session_id)
