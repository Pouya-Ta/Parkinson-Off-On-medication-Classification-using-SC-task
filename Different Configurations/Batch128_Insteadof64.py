"""
EEGNet-LSTM with batch size 128 — best single configuration run.
...
def make_config_name(cfg: dict) -> str:
    ...
    return (
        f"h{cfg['lstm_hidden']}_"
        f"do{str(cfg['dropout']).replace('.', 'p')}_"
        f"lr{lr_txt}_bs{BATCH_SIZE}_ep{N_EPOCHS}"
    )
...
def main():
    ...
    fold_df, epoch_df, summary_row = run_one_config(X, y, subject_ids, BEST_CONFIG)
